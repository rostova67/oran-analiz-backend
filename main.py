...
def find_similar_matches(
    ...
    try:
        ...
        condition = (
            history_df["MS1"].between(
                ms1 - SIMILARITY_TOLERANCE,
                ms1 + SIMILARITY_TOLERANCE
            )
            &
            history_df["MSX"].between(
                msx - SIMILARITY_TOLERANCE,
                msx + SIMILARITY_TOLERANCE
            )
            &
            history_df["MS2"].between(
                ms2 - SIMILARITY_TOLERANCE,
                ms2 + SIMILARITY_TOLERANCE
            )
            &
            history_df["UST_2_5"].between(
                ust - SIMILARITY_TOLERANCE,
                ust + SIMILARITY_TOLERANCE
            )
            &
            history_df["ALT_2_5"].between(
                alt - SIMILARITY_TOLERANCE,
                alt + SIMILARITY_TOLERANCE
            )
        )
...
def get_takimlar(
    ...
    for team_name, stats in team_stats.items():
        ...
        home_avg_goals = round(
            stats["home_goals"]
            / max(1, stats["home_matches"]),
            2
        )

        away_avg_goals = round(
            stats["away_goals"]
            / max(1, stats["away_matches"]),
            2
        )
